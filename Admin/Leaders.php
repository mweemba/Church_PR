...
$status=$_POST['status'];
 ...
 $stmt = $conn2->prepare($userqry);
											 ...
											 $userqry="SELECT * FROM `Leaders` INNER JOIN `churchmember` on `churchmember`.`ID Number`=`Leaders`.`Member_Id` INNER JOIN `department` on `department`.`Id`=`Leaders`.`Department_id` WHERE `PosStatus`<>'Deleted' ORDER BY `department`.`Name` ASC ";
											  $stmt = $conn2->prepare($userqry);
											  $stmt->execute();
												 }
...
//SELECT `id`, `perm_group_id`, `perm_group_name`, `short_code`, `enable_view`, `enable_add`, `enable_edit`, `enable_delete`, `created_at` FROM `permission_category` WHERE 1	
												...
												<ul class="dropdown-menu dropdown-default">
													<li>
														<a href="NewPosition.php?positionID=<?php echo $row['Position_id']; ?>">View Leader Details</a>
													</li>
													<li>
														<a href="profile2.php?=<?php echo $row['Member_Id'];?>">View Membership Details</a>
													</li>

													<li>
														<a href="NewPosition.php?positionID=<?php echo $row['Position_id']; ?>" >Edit</a>
													</li>
